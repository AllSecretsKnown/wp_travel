...
class WpTravlr {
	...
	public function wp_travel_menu() {
		...
		add_menu_page( 'WP Travel Settings', 'WP Travel', 'manage_options', 'wp_travel', array( &$this, 'wp_travel_options_page' ) );
	}
	...
	public function oc_connection_validate_options( $input ) {
		//Clean up out white listed prop
		$clean['wp_travel_ttl'] = wp_filter_nohtml_kses( trim( $input['wp_travel_ttl'] ) ); // Sanitize and trim text input (remove white space, strip html tags, and escape characters)

		//Make sure its an INT
		$clean['wp_travel_ttl'] = intval($clean['wp_travel_ttl']);

		//Make sure its an Int and that its greater than 3600
		if ( ! is_int( $clean['wp_travel_ttl'] ) OR $clean['wp_travel_ttl'] < 3600 ) {
			$clean['wp_travel_ttl'] = 3600;
			add_settings_error(
				'wp_travel_ttl',
				'wp_travel_ttlerror',
  ...
  <div class="wrap">
      ...
      <form action="options.php" method="post">
				...
				do_settings_sections( 'wp_travel' );
				?>
